subroutine time_integration(xl,x,y, vx,vy,vtot, mass, rho, p,c,itype, & 
...
double precision,intent(in) ::  mass(maxn),hsml ,dt ,xl 
double precision,intent(out) :: c(maxn),vtot(maxn) 
                             						 
integer :: i, itimestep,save_step
double precision ::dx(maxn),dy(maxn), dvx(maxn),dvy(maxn),avx(maxn),avy(maxn) ,drhodt(maxn)      
save_step = 100
