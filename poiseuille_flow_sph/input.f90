subroutine input(x, y, vx, vy,vtot,itype, mass, rho,p, hsml, ntotal,xl)

! subroutine to generate initial particle information

	implicit none
    include  "parameters.inc.txt" 
    double precision,intent(out) :: mass(maxn),rho(maxn),hsml,x(maxn),y(maxn),vx(maxn),vy(maxn),p(maxn),vtot(maxn),xl
	...
	call shear_cavity(x, y, vx, vy,vtot,itype, mass, rho,p, hsml, ntotal,xl)
	  do i = 1, ntotal 
...
subroutine shear_cavity(x, y, vx, vy,vtot,itype, mass, rho,p, hsml, ntotal,xl)
